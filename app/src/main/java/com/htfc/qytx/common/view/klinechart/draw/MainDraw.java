package com.htfc.qytx.common.view.klinechart.draw;
...
import com.htfc.qytx.common.view.klinechart.BaseKLineChartView;
import com.htfc.qytx.common.view.klinechart.KLineChartView;
...
import com.htfc.qytx.common.view.klinechart.entity.ICandle;
...
public class MainDraw implements IChartDraw<ICandle> {
    ...
    private Paint mTCLinePaint = new Paint(Paint.ANTI_ALIAS_FLAG);
    ...
    public MainDraw(BaseKLineChartView view) {
        ...
        mSelectorBackgroundPaint.setShadowLayer(ViewUtil.Dp2Px(context, 8), ViewUtil.Dp2Px(context, 1), ViewUtil.Dp2Px(context, 1), Color.parseColor("#26000000"));
    }
    ...
    private void drawSelector(BaseKLineChartView view, Canvas canvas) {
        ...
        float padding = ViewUtil.Dp2Px(mContext, 12);
        float margin = ViewUtil.Dp2Px(mContext, 10);
        float radius = ViewUtil.Dp2Px(mContext, 8);
        float width = 0;
        float left;
        float top = margin + view.getTopPadding();
        float height = padding * 4 + margin * 2 + textHeight * 5;

        ICandle point = (ICandle) view.getItem(index);
        List<String> strings = new ArrayList<>();
        strings.add("开盘: " + point.getOpenPrice());
        strings.add("收盘: " + point.getClosePrice());
        strings.add("最高: " + point.getHighPrice());
        strings.add("最低: " + point.getLowPrice());
        ...
        if (x > view.getChartWidth() / 2) {
            left = margin + view.getYAxisPaddingEnd() + view.getYAxisValueWidth();
        } else {
            left = view.getChartWidth() - width - margin ;
        }

        RectF r = new RectF(left, top, left + width, top + height);
        canvas.drawRoundRect(r, radius, radius, mSelectorBackgroundPaint);
        float y = top + margin + (textHeight - metrics.bottom - metrics.top) / 2;
