package com.htfc.qytx.common.view.klinechart;
...
import com.htfc.qytx.common.view.klinechart.base.IAdapter;
import com.htfc.qytx.common.view.klinechart.base.IChartDraw;
...
import com.htfc.qytx.common.view.klinechart.draw.MainDraw;
import com.htfc.qytx.common.view.klinechart.draw.Status;
import com.htfc.qytx.common.view.klinechart.entity.IKLine;
import com.htfc.qytx.common.view.klinechart.formatter.TimeFormatter;
import com.htfc.qytx.common.view.klinechart.formatter.ValueFormatter;
import com.htfc.qytx.common.view.klinechart.utils.ViewUtil;
import com.htfc.qytx.klinechart.R;
...
public abstract class BaseKLineChartView extends ScrollAndScaleView {
    ...
    private Paint mGridXPaint = new Paint(Paint.ANTI_ALIAS_FLAG);

    private Paint mGridYPaint = new Paint(Paint.ANTI_ALIAS_FLAG);
    ...
    private Path mSelectedYLinePath = new Path();

    private PathEffect mSelectedYLinePathEffect = new DashPathEffect(new float[] {10f, 10f}, 0);
    ...
    private Boolean isShowMaxAndMin = true;

    private float yAxisValueWidth = 0;
    private float yAxisPaddingEnd = 10;

    public float getYAxisValueWidth() {
        return yAxisValueWidth;
    }

    public float getYAxisPaddingEnd() {
        return yAxisPaddingEnd;
    }
    ...
    private void initRect() {
        if (isShowChild) {
            ...
            if (isShowVolume) {
                ...
                int mMainHeight = (int) (displayHeight * 1.0f);
                int mVolHeight = 0;
                mMainRect = new Rect(0, mTopPadding, mWidth, mTopPadding + mMainHeight);
    ...
    private void drawGird(Canvas canvas) {
        ...
        for (int i = 0; i <= mGridRows; i++) {
            canvas.drawLine(yAxisValueWidth + yAxisPaddingEnd, rowSpace * i + mMainRect.top, mWidth, rowSpace * i + mMainRect.top, mGridXPaint);
        }
        //-----------------------下方子图------------------------
        if (mChildDraw != null) {
            canvas.drawLine(0, mVolRect.bottom, mWidth, mVolRect.bottom, mGridXPaint);
            canvas.drawLine(0, mChildRect.bottom, mWidth, mChildRect.bottom, mGridXPaint);
        } else {
            if (mVolDraw != null) {
                canvas.drawLine(0, mVolRect.bottom, mWidth, mVolRect.bottom, mGridXPaint);
            }
        ...
        //纵向的grid
        float columnSpace = (mWidth - yAxisValueWidth - yAxisPaddingEnd) / mGridColumns;
        for (int i = 1; i < mGridColumns; i++) {
            canvas.drawLine(columnSpace * i + yAxisValueWidth + yAxisPaddingEnd, 0, columnSpace * i + yAxisValueWidth + yAxisPaddingEnd, mMainRect.bottom, mGridYPaint);
            canvas.drawLine(columnSpace * i + yAxisValueWidth + yAxisPaddingEnd, mMainRect.bottom, columnSpace * i + yAxisValueWidth + yAxisPaddingEnd, mVolRect.bottom, mGridYPaint);
            if (mChildDraw != null) {
                canvas.drawLine(columnSpace * i + yAxisValueWidth + yAxisPaddingEnd, mVolRect.bottom, columnSpace * i + yAxisValueWidth + yAxisPaddingEnd, mChildRect.bottom, mGridYPaint);
            }
        }
    }
    ...
    private void drawK(Canvas canvas) {
        ...
        //画选择线
        if (isLongPress) {
            mSelectedYLinePath.reset();
            IKLine point = (IKLine) getItem(mSelectedIndex);
            float x = getX(mSelectedIndex);
            float y = getMainY(point.getClosePrice());
            // k线图竖线
            mSelectedYLinePaint.setPathEffect(mSelectedYLinePathEffect);
            mSelectedYLinePath.moveTo(x, mMainRect.top);
            mSelectedYLinePath.lineTo(x, mMainRect.bottom);
            canvas.drawPath(mSelectedYLinePath, mSelectedYLinePaint);
    ...
    private void drawText(Canvas canvas) {
        ...
        //--------------画上方k线图的值（Y轴坐标）-------------
        if (mMainDraw != null) {
            //x : mWidth - calculateWidth(formatValue(mMainMaxValue)) 第一个值
            //y : baseLine + mMainRect.top
            //canvas.drawText(formatValue(mMainMaxValue), 0, mMainRect.top + baseLine / 2, mTextPaint);
            //x : mWidth - calculateWidth(formatValue(mMainMinValue)) 最后一个值
            //y : mMainRect.bottom - textHeight + baseLine
            //canvas.drawText(formatValue(mMainMinValue), 0, mMainRect.bottom + textHeight / 2 - baseLine / 2, mTextPaint);
            float rowValue = (mMainMaxValue - mMainMinValue) / mGridRows;
            float rowSpace = mMainRect.height() / mGridRows;
            for (int i = 0; i <= mGridRows; i++) {
                String text = formatValue(rowValue * (mGridRows - i) + mMainMinValue);
                //y : fixTextY(rowSpace * i + mMainRect.top)
                canvas.drawText(text, 0, fixTextY1(rowSpace * i + mMainRect.top), mTextPaint);
                //x : mWidth - calculateWidth(text)
            }
...
//        //最后一个时间
//        if (translateX >= startX && translateX <= stopX) {
//        canvas.drawText(getAdapter().getDate(mStopIndex), mWidth - mTextPaint.measureText(getAdapter().getDate(mStopIndex)), y, mTextPaint);
//        }
...
//        }
        ...
        for (int i = 0; i <= mGridColumns; i ++) {
            if (i == 0) {
                ...
                interval = (mWidth - yAxisValueWidth - yAxisPaddingEnd - (textLen * (mGridColumns + 1))) / mGridColumns;
                canvas.drawText(text, xTime + yAxisValueWidth + yAxisPaddingEnd, y, mTextPaint);
                xTime += textLen;
            } else if (i == mGridColumns) {
                String text = getAdapter().getDate(mStopIndex);
                float textLen = mTextPaint.measureText(text);
                canvas.drawText(text, mWidth - textLen, y, mTextPaint);
            } else if (isFullScreen()) {
                float gridX = getX((int) ((columnSpace * i) / mPointWidth));
                translateX = xToTranslateX(gridX);
                int indexPoint = indexOfTranslateX(translateX);
                String text = getAdapter().getDate(indexPoint);
                float textLen = mTextPaint.measureText(text);
                xTime += interval;
                canvas.drawText(text, xTime + yAxisValueWidth + yAxisPaddingEnd, y, mTextPaint);
                xTime += textLen;
                ...
                //数据不能占满整个宽度
                if (mItemCount > 1) {
                    float gridX = stopX * mScaleX;
                    translateX = xToTranslateX(gridX);
                    int indexPoint = indexOfTranslateX(translateX);
                    String timeText = getAdapter().getDate(indexPoint);
                    float textLen = mTextPaint.measureText(timeText);
                    if (timeText != null) {
                        if (gridX + textLen / 2 > mWidth) {
                            canvas.drawText(timeText, mWidth - yAxisValueWidth - yAxisPaddingEnd - textLen, y, mTextPaint);
                        } else if (gridX < 3 * textLen / 2) {
                            canvas.drawText(timeText, textLen, y, mTextPaint);
                        } else {
                            canvas.drawText(timeText, gridX - textLen / 2, y, mTextPaint);
                        }
                    }
                }
                break;
...
//            canvas.drawText(date, x - textWidth / 2, y + baseLine + 5, mTextPaint);
    ...
    private void calculateValue() {
        ...
        for (int i = 0; i <= mGridRows; i++) {
            String text = formatValue(rowValue * (mGridRows - i) + mMainMinValue);
            yAxisValueWidth = Math.max(yAxisValueWidth, mTextPaint.measureText(text));
        }
        ...
        mStartIndex = indexOfTranslateX(xToTranslateX(yAxisValueWidth + yAxisPaddingEnd));
        mStopIndex = indexOfTranslateX(xToTranslateX(mWidth - yAxisPaddingEnd));
    ...
    private float getMinTranslateX() {
        return isFullScreen() ? -mDataLen + mWidth / mScaleX - mPointWidth / 2 : 0;
    }
    ...
    public float getX(int position) {
        return position * mPointWidth + yAxisPaddingEnd + yAxisValueWidth;
    }
    ...
    public void hideMaxAndMin() {
        isShowMaxAndMin = false;
        invalidate();
    }
    ...
    public void setGridLineWidth(float width) {
        mGridXPaint.setStrokeWidth(width);
    }

    /**
     * 设置表格线颜色
     */
    public void setGridXLineColor(int color) {
        mGridXPaint.setColor(color);
    }

    /**
     * 设置表格线颜色
     */
    public void setGridYLineColor(int color) {
        mGridYPaint.setColor(color);
    }
